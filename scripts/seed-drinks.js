...
// Note: This script should be run with ts-node or converted to TypeScript
// For now, using a workaround with dynamic imports
...
const brandsData = [
  {
    ...
    fakePatterns: [
      { pattern: '^00000', notes: 'Fake: All zeros pattern', risk_level: 'high' },
      { pattern: '^12345', notes: 'Fake: Sequential numbers', risk_level: 'high' },
      { pattern: '^FAKE', notes: 'Fake: Contains FAKE', risk_level: 'high' },
    ],
  },
  {
    name: 'Remy Martin',
    ...
    fakePatterns: [
      { pattern: '^00000', risk_level: 'high' },
      { pattern: '^TEST', risk_level: 'high' },
    ],
  },
  {
    name: 'Martell',
      ...
      for (const patternData of brandData.fakePatterns) {
        ...
        if (!existingPattern) {
          await FakePattern.create({
            brand_id: brand._id,
            pattern: patternData.pattern,
            fake_signs: patternData.fake_signs || ['Suspicious pattern detected'],
